...
def save_audio(audio: torch.Tensor, filename: str):
    ...
    if audio is not None:
        ...
        wavfile.write(filename, 24000, audio_cpu)  # Kokoro uses 24kHz sample rate
...
def main():
    ...
    try:
        ...
        results = list(
            pipeline.generate_from_tokens(tokens=phonemes, voice="af_bella", speed=1.0)
        )
        if results:
        ...
        for result in pipeline.generate_from_tokens(
            tokens=tokens, voice="af_bella", speed=1.0
        ):
            ...
            if result.tokens:
                for token in result.tokens:
                    if hasattr(token, "start_ts") and hasattr(token, "end_ts"):
                        print(
                            f"Token: {token.text} ({token.start_ts:.2f}s - {token.end_ts:.2f}s)"
                        )
